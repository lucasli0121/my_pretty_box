...
import com.effectsar.labcv.core.opengl.ProgramManager;
import com.effectsar.labcv.core.opengl.ProgramTextureOES;
import com.effectsar.labcv.core.opengl.ProgramTextureYUV;
...
public class ImageUtil {
    ...
    private void initFrameBufferIfNeed(int width, int height) {
        boolean need = null == mFrameBufferShape || mFrameBufferShape.x != width || mFrameBufferShape.y != height;
    ...
    public void release() {
        ...
        if (mTextureHandles != null) {
            GLES20.glDeleteTextures(1, mTextureHandles, 0);
     ...
     * @param mUVMatrix            matrix used for surface texture input
    ...
    public  int transferTextureToTexture(int inputTexture, EffectsSDKEffectConstants.TextureFormat inputTextureFormat,
                                         EffectsSDKEffectConstants.TextureFormat outputTextureFormat,
                                         int width, int height, Transition transition, float[] mUVMatrix) {
        if (outputTextureFormat != EffectsSDKEffectConstants.TextureFormat.Texure2D){
            LogUtils.e("the inputTexture is not supported,please use Texure2D as output texture format");
            return  GlUtil.NO_TEXTURE;
        }
        ...
        boolean targetRoated = (transition.getAngle()%180 ==90);
        if (inputTextureFormat == EffectsSDKEffectConstants.TextureFormat.Texture_Oes) {
            ProgramTextureOES oesProgram = (ProgramTextureOES)mProgramManager.getProgram(inputTextureFormat);
            // 传入uv matrix，该矩阵从视频流中获取，仅在oes转2d纹理时会使用
            return oesProgram.drawFrameOffScreen(inputTexture, targetRoated?height:width, targetRoated?width:height, transition.getMatrix(), mUVMatrix);
        }
        return mProgramManager.getProgram(inputTextureFormat).drawFrameOffScreen(inputTexture, targetRoated?height:width, targetRoated?width:height, transition.getMatrix());
    }

    /** {zh}
     * @param texture             输入结果纹理
     * @param src_texture         输入原纹理 2D
     * @param progress            对比横轴
     * @param inputTextureFormat  输入纹理格式 2D
     * @param width               输入纹理的宽
     * @param height              输入纹理的高
     * @param transition          纹理变换方式
     * @return 输出纹理 2D
     ...
     * @param src_texture          src texture, 2D
     * @param progress             compare progress
     * @param width                input texture width
     * @param height               input texture height
     * @param transition           texture transformation mode
     * @return  output texture 2D
     * @brief  texture to texture
     */
    public  int transferTextureToTextureForCompare(int texture, int src_texture, float progress, EffectsSDKEffectConstants.TextureFormat inputTextureFormat,
                                         int width, int height, Transition transition) {
        if (inputTextureFormat != EffectsSDKEffectConstants.TextureFormat.Texure2D){
            LogUtils.e("the inputTexture is not supported,please use Texure2D as output texture format");
            return  GlUtil.NO_TEXTURE;
        }
        if (null == mProgramManager) {
        ...
        boolean targetRoated = (transition.getAngle()%180 ==90);
        return mProgramManager.getCompareProgram().drawFrameOffScreenForCompare(texture, src_texture, progress, targetRoated?height:width, targetRoated?width:height, transition.getMatrix());
     ...
     * @param width         width
     * @param height        height
     * @param consistent    whether buffer update on the same texture
     * @return  output texture
    ...
    public int transferBufferToTexture(ByteBuffer buffer, EffectsSDKEffectConstants.PixlFormat inputFormat,
                                EffectsSDKEffectConstants.TextureFormat outputFormat, int width, int height, boolean consistent){
    ...
    public Bitmap transferBufferToBitmap(ByteBuffer buffer, EffectsSDKEffectConstants.PixlFormat format,
                                  int width, int height){
        if (format != EffectsSDKEffectConstants.PixlFormat.RGBA8888) {
