...
public class SimplePlayer {
    ...
    SoftReference<IPlayStateListener> mListener;
    ...
    private VideoBufferDecodeThread mVideoBufferDecodeThread;
    ...
    public void setPlayStateListener(IPlayStateListener mListener) {
        this.mListener =  new SoftReference<IPlayStateListener>(mListener);
    }
    ...
    public void play() {
        ...
        if (mAudioOn) {
            ...
            if (null != mAudioDataListener) {
                mAudioDataListener.onNoAudioAvaliable();
            }
        }
        if (mVideoBufferDecodeThread == null) {
//            mVideoBufferDecodeThread = new VideoBufferDecodeThread();
//            mVideoBufferDecodeThread.start();
        }
    }
    ...
    private class VideoDecodeThread extends Thread {
        ...
        public void run() {
            ...
            if (videoTrackIndex >= 0) {
                ...
                LogUtils.e("framerate" + frameRate);
                ...
                if (mListener.get() != null) {
                    mListener.get().videoAspect(videoRotation % 180 == 0 ? width : height, videoRotation % 180 == 0 ? height : width, videoRotation);
                    mListener.get().frameRate(frameRate);
                }
    ...
    private class VideoBufferDecodeThread extends Thread {
        @Override
        public void run() {
            MediaExtractor videoExtractor = new MediaExtractor();
            MediaCodec videoCodec = null;
            ...
            if (videoTrackIndex >= 0) {
                ...
                try {
                    videoCodec = MediaCodec.createDecoderByType(mediaFormat.getString(MediaFormat.KEY_MIME));
                    //  {zh} 这里的区别就是surface 变为了null 不需要写入到一个surface, 而是直接读yuv buffer  {en} The difference here is that the surface becomes null without writing to a surface, but directly reading the yuv buffer
    ...
    private class AudioDecodeThread extends Thread {
        ...
        public void run() {
            ...
            if (audioCodec == null) {
                ...
                if (audioTrackCount == 0) { // without audio track, notify null to enable writer
                    if (mAudioDataListener != null) {
                        mAudioDataListener.onNoAudioAvaliable();
                    }
                }
